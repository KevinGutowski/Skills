...
**Sources:** [references/sources.md](rails-testing/sources.md) — 6 Evil Martians TestProf-lineage posts (canonical playbook: https://test-prof.evilmartians.io/playbook).
...
- **[optimizing-rails.md](optimizing-rails.md)** — runtime/production performance (N+1, caching, Puma); this skill owns the *test suite* as the artifact. Shared vocabulary (StackProf, Speedscope) but different entry points and fixes.
- **[dhh-style.md](dhh-style.md)** — 37signals tests with fixtures and Minitest; this corpus is factory_bot/RSpec-centric and argues "use both." Different schools — route by the project's existing choice, don't blend. The fixtures school's own playbook is [rails-fixtures-testing.md](rails-fixtures-testing.md); its coverage-ownership heuristic (assert each behavior once, at the layer that owns it) is written in fixtures-school idiom but the *discipline* — don't restate one layer's assertion at another — reads across to factory suites too.
- **`working-with-ai` (agentic-coding)** — encode this skill's nudges ("replace `let` with `let_it_be`", "don't create 50 records to test pagination") as project rules; fast reliable tests are its feedback-loop layer.

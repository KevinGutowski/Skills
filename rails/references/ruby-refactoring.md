...
The governing rule: **"Don't treat code smells as bugs. It will be a waste of time to 'fix' every smell. Not every smell is the symptom of a problem."** Smells are review-time indicators; refactoring can itself introduce new smells. ⚠️ *School note:* Ruby Science is the **extraction school** — it lists Callback as a smell and prescribes class extraction where [dhh-style.md](dhh-style.md) keeps fat models and treats callbacks as idiomatic. Apply within the project's chosen school (see [rails-event-sourcing.md](rails-event-sourcing.md)'s router); on a 37signals-style codebase, use the *diagnosis* half and temper the extraction prescriptions.
...
- **[dhh-style.md](dhh-style.md)** — the competing doctrine on callbacks and extraction; diagnosis transfers, prescriptions route by school.

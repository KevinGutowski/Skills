...
1. **Go wide, remix, repeat (exploration):** be specific about features and mood, and "most importantly, ask for multiple options" — e.g. 10 variations, each as its own named `#Preview`; then remix the promising ones. Make options *structurally different concepts, not variants* (Josh Puckett); current tooling is depth-biased, so budget divergence deliberately (Ian Silber). A forcing recipe (0xdesign Design Lab): one variant per named axis — information hierarchy / layout model / density inversion / interaction model / expressive — and first infer the project's own tokens (tailwind config, `:root` variables, 2-3 existing buttons/cards/forms) into a design brief so **variants explore structure, the project supplies the skin**. Range moves + variant-lab mechanics: [references/agents-field-notes.md](references/agents-field-notes.md).
...
- **Mock-to-code: diff against the mock element-by-element** (Stamatiou vs Claude Design 2026, first pass "65/100") — the checklist below — before iterating on anything else. Judge overflow/overlap in rendered browser screenshots, not DOM math — "`scrollHeight` checks alone are not enough because grid panels can visually cover each other" (Zara Zhang, frontend-slides).
...
- **Variant review = click-to-comment on the rendered artifact** (0xdesign Design Lab): serialize feedback to the agent as CSS selector + element description + comment, grouped per variant, plus one *required* overall-direction verdict — synthesize from the verdict first, relocate elements selector-first with the description as fallback.
- **Show taste, don't ask for it — and keep previews authentic** (Zara Zhang, frontend-slides): "people discover what they want by seeing it" — render 2-3 real previews (one safe, one bold, one wildcard) seeded with the user's actual content and logo instead of asking abstract taste questions; never render workflow metadata on the artifact (no "Option A", template names, requirement notes) — only real product chrome.

...
- *Dive Club podcast interviews, 2025–26: Brian Lovin (Notion, dvEwb1Ajkwo), Geoffrey Litt (zJf0UeCwQqE), Steve Ruiz (tldraw, 3SvL0r-Lhh8), Luis Ouriach (Pn2G7JhxNKc), Kyle Santos (HcLz3ikw-n0), Josh Puckett (wym3V9FycTk), Andy Madrick (Notion, IfPK0LwbX_0), Katie Dill (Stripe, Dpy-yyYXhgU) — the Dive Club field notes section + references file.*
...
**Dive Club field notes (2025–26)** — working-designer patterns for this tier, distilled (full quotes: [references/agents-field-notes.md](references/agents-field-notes.md)):
- **The shared playground** (Brian Lovin, Notion): an 80/20 component kit in one shared codebase — close-enough sidebar/buttons, not pixel-perfect — where designers "yoink cool interactions from other people's prototypes" and each pushes fidelity only where their question demands it (Will Dawson rebuilt "a very simplified version of the Notion editor" because inline-AI feel needs a real editor). Collaboration moves "from, 'Hey, check out this Figma URL.' to, 'Hey, check out this deploy preview.'"
- **Jigs** (Geoffrey Litt): "I call these tools jigs from woodworking" — disposable one-project tools an agent makes economical, up to a migration "command center" that visualizes a port you'd otherwise run blind.
- **Visual prompting** (Steve Ruiz, tldraw Make Real): screenshot or sketch *is* the prompt (the "4,000-year-old senior web developer who loves their designers" persona); iterate by drawing arrows on the output; paste a reference — "make this look like… stripe.com."
- **Start from anywhere** (Luis Ouriach): "You might start a document, you might start in a canvas, you might start in the front end" — the payoff is "not faster, but further"; "the canvas is very important for ideation… but the browser is where we commit."
- **Effect studios + on-demand controls** (Kyle Santos): when hunting a feel, "I just ended up building a studio basically that gave me a bunch of options"; a global-memory recipe makes tuning panels one phrase — "Whenever I say give me controls… go grab the [Leva] component."
- **Storyboard-as-prompt** (Josh Puckett): paste a Figma storyboard PNG — "here's a storyboard of like what I want to create"; don't dial fidelity there because "Figma's a lie. It's not your actual code base"; his own frames were "too high fidelity. They should just be color blocks."
- **Editable stakeholder prototypes → production** (Andy Madrick, Notion): ship prototypes with inline-editable text so stakeholders change titles/headers and screenshot back — "we're all like co-designing together… still like on Apple's rails"; threshold: "the bigger the feature, the more likely I am to start in code"; hedge by building variants (three modals shipped to test — "I hate to say that the safe one did" win); a playground directory fed to an agent — "Make this work in production" — one-shotted the port; and the source of truth moves: "you'd be surprised how many Figma files in Notion are just screenshots of the product."
- **Context-first prototyping** (Katie Dill, Stripe — Protodash): prototype inside the surrounding product, since "no product is like ever felt by your user in isolation"; and review accordingly — "Please don't present. Please don't pitch… I just want to see it like a user."

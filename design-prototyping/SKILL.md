...
description: "Prototype and present design work across three tiers — fake-it (Keynote Magic Move), real code (SwiftUI as a design tool), and agents (collaborators, never designers). Use when validating an idea before building, exploring or converging on UI variants, planning feedback sessions, or structuring a review or pitch. Triggers: prototype, fake it, Magic Move, SwiftUI prototype, tuning panel, design variants, converge on a look, present design work."
---
...
5. **Converge by walking the design tree (convergence):** after going wide, render 3–5 *structurally different* variants behind a floating live switcher *inside the app* — "a throwaway route on its own is a vacuum: every variant looks fine in isolation" (Pocock /prototype; Will Ness's grilling-frontend variant, 2026). Then rounds of pick-favorites-plus-verdict, each round re-prototyping the *current* question one level deeper: overall design → component groups → individual components — until the feature is designed in detail. Switcher mechanics, caps, capture rules: [references/agents-field-notes.md](references/agents-field-notes.md).

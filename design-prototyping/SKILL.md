...
- **Prototype at true spatial context** (Jae Park): for anything viewed off-desk, fake it at real distance/angle. Full physical-tier methods (mockups, rigs, jigs) → `hardware-product-design`. **Scaffold, don't script, for collaborators** (PBJ) and **prototype the core loop before the app exists** (CapWords — "we didn't even have an app yet"). **Figma diverges, code decides** (Castilho): motion and interaction feel are *never* prototyped in Figma — always tuned in code. Full field notes: [references/methods.md](references/methods.md).

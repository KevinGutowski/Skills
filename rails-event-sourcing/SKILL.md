...
**Sources** — this skill aggregates twelve Arkency posts (blog.arkency.com; the consultancy "called in to fix existing large Rails applications"; tooling: RailsEventStore + aggregate_root + the open-source `ecommerce` reference app):
- *"The Rails Way in 2026" (Krzywda, 2026) — the positioning piece. /the-rails-way-in-2026/*
- *"Storing data as a sequence of events" (Reszke, 2026). /storing-data-as-a-sequence-of-events-ruby-conf-austria-lightning-talk-supplement/*
- *"Fast introduction to Event Sourcing for Ruby programmers" + "Why use Event Sourcing" (Rybczyński/Prągłowski, 2015)*
- *"One simple trick to make Event Sourcing click" (Pacana, 2017) — the aggregate pattern. /one-simple-trick-to-make-event-sourcing-click/*
- *"Why Event Sourcing basically requires CQRS and Read Models" (Pankowecki, 2017)*
- *"Process Managers revisited" (Pacana, 2018) + "Heuristics for choosing bounded context for an event handler" (Łasocha, 2019)*
- *"Ultimate guide to 3rd party calls from your Aggregate" (Fiedler, 2020)*
- *"Rails apps have layers but no modules" (Krzywda, 2026) + "Physical separation in Rails apps" (2017)*
- *"Event-sourcing whole app — opinions" (2017), "Audit log with event sourcing" (2021), "Legacy Rails DDD Migration strategy" (2020), "Beyond the Rails Way" (2014)*
...
1. **`dhh-style` (vanilla — the default).** Arkency themselves: the Rails Way is "great at the beginning, when you're still not sure where you go with the features." Stay here while entities are mostly entities. (They review 37signals' Fizzy respectfully: concerns "applied intentionally rather than as accidental accumulation.")
2. **`layered-rails` (extraction)** when controllers/models bloat but the domain is one coherent module. Arkency's honest take: service objects "are a good solution to the transaction boundary… It's not bad" — but "service objects do nothing about modularisation… just another horizontal slice."
...
- **`dhh-style`** — the default school; this one is the escalation for process-heavy domains. Arkency teaches the Rails Way first and endorses it early-stage.
- **`layered-rails`** — its service objects are this school's on-ramp step 1; this school adds the vertical (modules) axis layers can't.

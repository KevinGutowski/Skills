---
name: dhh-style
description: "Write Ruby and Rails code following DHH's 37signals conventions — REST-pure controllers, fat models with concerns, Current attributes, Hotwire, the Solid suite, vanilla-stack dependency discipline ('fight hard before adding Ruby dependencies'), delegated types (the Recording/Recordable architecture), and database-per-tenant multi-tenancy. The DEFAULT Rails school: start here; escalate to other schools only on named pains (see rails-event-sourcing's router; for smell-driven extraction doctrine see ruby-refactoring — a competing school). Use when writing Rails controllers, models, views, or reviewing Rails code on a vanilla/37signals-style codebase, or on mentions of DHH, 37signals, Basecamp, HEY, Campfire, Fizzy. Triggers: Rails Way, vanilla Rails, fat models, concerns, REST controllers, Current attributes, Hotwire, Turbo, Stimulus, Solid Queue, Solid Cache, no build, import maps, delegated types, multi-tenancy, Kamal, 37signals."
---

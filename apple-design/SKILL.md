...
- **Canonical current specs → `hig`** — talk-derived references here carry reasoning and craft; when you need Apple's exact letter-of-the-law spec (or a staleness tiebreaker), fetch the live HIG.
- **Typography split:** Apple platforms → this skill's apple-typography reference; web/CSS → `web-typography`. Never cross-apply platform conventions.
- **Accessibility split:** Apple platforms → this skill's apple-visual-accessibility reference; web → `web-accessibility` (same split as typography).

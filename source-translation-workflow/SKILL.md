...
For the mechanical OpenAI ASR pass, including chunking long recordings, preserving `openai_chunk_###.json`, detecting hallucinated loops, and retrying only failed chunks, use `openai-transcription-chunking` first. Return here after the working transcript is assembled.
...
Use `catalog-reconciliation-research` after this skill when translated excerpts need to become database records, source links, local archive pointers, site content, or cross-linked artist/artwork evidence.

This skill produces trustworthy text layers. Catalog reconciliation decides how those layers become structured records.

...
**Follow-up verification (2026-07-10):** all six repository gates pass across the current 38-skill corpus. The follow-up found and corrected one evaluation-integrity defect: the smoke-results artifact still reported 39 probes after the deleted `goal` skill was removed, while its checker only validated result-table coverage behind a hard-coded `39/39` match. The checker now derives the expected result summary, fixture count, and table size from the live skill inventory.

**Manual body pass (2026-07-10):** every current `SKILL.md` was read in full after the mechanical follow-up. Four additional defects were corrected: `frontend-design`'s early absolute ban on system fonts contradicted its later zero-network fallback; `notification-design` routed to a nonexistent *Utility* principle instead of *Purpose*; `user-research` named the nested developer-tool GTM reference as though it were top-level; and `route-planning` alone used folded, unquoted description YAML despite the repository's single-line quoted convention. The validator now enforces that convention, and the route guide no longer claims Google Maps has no observed waypoint cap.

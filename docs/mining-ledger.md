...
## 2026-07-07 — ui-skills.com registry sweep
Ticket-driven review of ui-skills.com (Julien Thibeaut's design-engineering skill registry, github.com/ibelick/ui-skills). Full manifest, fold map, conflicts, and parked items in `docs/ui-skills-source-mining-2026-07.md`. Screened all ~80 registry entries: 5 first-party ibelick skills read in full; ~18 in-scope external sources fetched and screened (saved under /tmp/uiskills-screen/); framework-engineering suites (antfu/vue/three.js/next/svelte/RN/playwright) logged out-of-scope. Five registry entries were already in the library (frontend-design, emil-design-eng, make-interfaces-feel-better — re-verified superset, oklch-skill, vercel web-design-guidelines). Folded across 9 clusters: `web-design` (NEW web-sound reference + render-pipeline/View-Transitions/a11y-audit folds; description gained "UI sound feedback", fresh-judge probes 3/3), `working-with-ai` (ai-ui-direction de-slop schools: baseline-ui, deslop two-pass, model-tell catalogs), `frontend-design` (impeccable + taste-skill v2 + interface-design: Design Read, checkable slop tells dated 2026, Jane Doe authenticity, DS routing), `design-craft` (Budge token-snapped live tuning, elevation arithmetic, drift taxonomy), `motion` (AnimatePresence pitfalls), `swiftui` (dimillian practitioner patterns), `design-prototyping` (design-lab variant axes/feedback overlay, frontend-slides show-don't-tell), `creating-skills` (NEW published-skill-field-patterns reference: distribution model, linter-skill format, design-memory convergence, remote-fetch supply-chain cautions), ops doc (remote-fetch intake red flag). Parked: ibelick fixing-metadata (page-metadata seed, needs corroboration), Salaja to-spring-or-not-to-spring (unscreened), Rauno Invisible Details pointer, presentations task shape. Conflicts recorded, not imported: exit ease-in (Salaja), ask-first gating (bencium), category-reflex CSVs (ui-ux-pro-max). Pre-existing `check_vercel_overhaul.py` failure (extra=['goal']) reproduced on clean main — separate fix needed.

## 2026-07-07 — ui-skills conflict resolutions
Kevin ratified the conflict-resolution ladder for the ui-skills sweep (school structure only for multi-source positions with a project-level routing condition and real misrouting blast radius; otherwise scope split → in-reference divergent-tradition note → reject). Applied: exit easing became an accelerate-on-exit tradition note in `web-design` (web-animation-design §ease-in) with a follow-the-project rule; token naming stays the brand-layer/system-layer scope split already folded; ask-first gating and category-reflex CSVs stay rejected; exit-matches-initial stays ledger-only pending a second source. Resolution policy recorded in `docs/ui-skills-source-mining-2026-07.md` §Conflicts.

...
Third review pass: Kevin showed multiple matrix examples that still did not make sense out of context. Pruned the gallery from 45 to 32 examples instead of continuing to patch vague sketches. Removed supplemental cards whose visuals failed to prove the named rule (visual hierarchy, button prominence, relative sizing, type scale, practical fonts, line length, links, readability alignment, letterspacing, not-color-alone, flat depth). Lesson for future image-heavy mining: a code-native recreation only earns a slot when the before/after is self-evident without the book page beside it.

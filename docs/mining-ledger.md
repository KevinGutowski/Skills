...
## 2026-08-13 — The 37signals Manager Playbook (basecamp.com/managers)

Ticket-driven mine of the full 16-chapter public web book plus its index, fetched with `curl` and normalized locally (working text kept out of the repo). Density HIGH; no new top-level skill — `people-management` already owns the task shape, and the source's value is the operating-manual layer the Scott/Zhuo/Hogan books are thinnest on. Created two references inside `people-management` because nothing owned those shapes: `performance-cadence.md` (SEC model + HIGH/MID/LOW scans and the two-lows escalation rule, the 3/6/12 review calendar ending in a named retain decision, first-year ramp curriculum, cycle reviews, review-writing criteria incl. the "how much red pen is there?" hidden-subsidy question, designed peer-feedback questions, attainable/stretch goal split, the manager's own SEC standard, and AI as a bounded prep lens) and `authority-and-escalation.md` (time off as inform-not-request, leave programs as escalate-never-advise, the complaints hard stop and its script, comp/profit-share with no manager input, capacity hand-off, and a 10-item escalation checklist). Extended `feedback-and-guidance.md` (approach/tactics/engagement surfaces, micromanagement boundary, ❌/✅ pairs), `growth-and-careers.md` (directive/non-directive/situational coaching, promotion's performance + business gates, recognition mechanics), `hiring-and-firing.md` (open-call hiring, take-home exercise design, the underperformance path with HR-before-report escalation order, recovery *and* graduation, both conversation scripts), `meetings-and-comms.md` (1:1 cadence and documentation), plus `sources.md`, `coverage-gaps.md`, SKILL.md router/description (447 chars), and the README row. Cross-skill fold: `design-org-influence/references/promotion-and-reviews.md` gained the decider's-side promotion criteria. 114 quotes verified via `verify_quotes.py`, zero misses; illustrative utterances converted to italics so quotation marks keep meaning verified text. Conflict recorded, not blended: 37signals teaches open "why" questions in non-directive coaching where Hogan avoids *why* as judgy — scoped as two schools, pick per relationship. Skipped/parked with reasons: `shape-up` (cycle boundaries are a management beat, not a shaping lesson), `working-with-ai` (AI-prep guidance is inseparable from HR-sensitive context), `learning-experience-design` (onboarding topic list, not a teaching method), and all internal-tool/policy specifics (BambooHR/Jotform flows, dollar figures, leave thresholds) which stay labeled as one org's settings. Full manifest, fold map, source-to-rule map, and routing probes: `docs/37signals-manager-playbook-source-mining-2026-08.md`.

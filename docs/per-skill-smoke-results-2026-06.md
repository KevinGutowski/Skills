...
| S17 | `goal` | `goal` |  | Pass | Explicit `/goal` invocation and thread-focus behavior. |
| S18 | `graphics-fundamentals` | `graphics-fundamentals` |  | Pass | Color spaces, rasterization, compression, and display rendering behavior. |
| S19 | `hardware-product-design` | `hardware-product-design` |  | Pass | Wearable enclosure, input method, prototyping, manufacturing tolerances. |
| S20 | `learning-experience-design` | `learning-experience-design` |  | Pass | Workshop, job aid, practice, feedback, and transfer checks. |
| S21 | `logo-design` | `logo-design` |  | Pass | Logo concept, brand nouns, sketches, black-and-white presentation. |
| S22 | `malleable-software` | `malleable-software` | `ai-experience-design` | Pass | Reshapable product surface and plugins are primary; AI-assisted changes may require AI UX patterns. |
| S23 | `motion` | `motion` |  | Pass | Motion spring animation, easing options, and MotionScore. |
| S24 | `notification-design` | `notification-design` |  | Pass | Push strategy, permissions, fatigue, grouping, and Apple Watch. |
| S25 | `people-management` | `people-management` |  | Pass | New manager, underperforming direct report, 1:1, feedback, career follow-up. |
| S26 | `photographic-lighting` | `photographic-lighting` |  | Pass | Product photo lighting, glossy object glare, reflections, family-of-angles tests. |
| S27 | `product-decision-making` | `product-decision-making` |  | Pass | Build-or-not decision using defaults, onboarding, settings, metrics, and judgment. |
| S28 | `rails` | `rails` |  | Pass | Rails app performance, Turbo, N+1, migrations, Sidekiq production failures. |
| S29 | `research-cataloging` | `research-cataloging` |  | Pass | Exhaustive source sweep, Wayback, OCR, captions, ledger, transcript readiness. |
| S30 | `route-planning` | `route-planning` |  | Pass | Multi-stop delivery route optimization and backtracking diagnosis. |
| S31 | `shape-up` | `shape-up` |  | Pass | Six-week pitch, appetite, fat-marker scope, risks, betting-table framing. |
| S32 | `swiftui` | `swiftui` |  | Pass | @State resets, LazyVStack performance, gestures, spring transition. |
| S33 | `user-onboarding` | `user-onboarding` | `ux-writing` | Pass | Empty dashboard and first-run setup are primary; welcome copy may need UX writing. |
| S34 | `user-research` | `user-research` |  | Pass | Research plan, screener, field guide, method choice. |
| S35 | `ux-writing` | `ux-writing` |  | Pass | Validation errors, labels, button text, plan-name microcopy. |
| S36 | `web-design` | `web-design` |  | Pass | CSS typography, forms, accessibility states, OKLCH colors, animation values. |
| S37 | `working-with-ai` | `working-with-ai` |  | Pass | AGENTS.md rules, multi-agent review loop, secret safety, coding agents. |
| S38 | `write-clear-prose` | `write-clear-prose` |  | Pass | Essay line edit for clarity, rhythm, structure, plain language, voice preservation. |
| S39 | `x-post-reader` | `x-post-reader` |  | Pass | Public x.com URL plus recent related posts. |
...
- S22: `malleable-software` is first for reshapable product surfaces; `ai-experience-design` is a plausible helper for AI-assisted change behavior.
- S33: `user-onboarding` is first for the empty dashboard and first-run path; `ux-writing` is a plausible helper for welcome copy.

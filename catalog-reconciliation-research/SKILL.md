...
Use this after or alongside `archival-research`. The archival skill gathers the source trail broadly; this skill turns that trail into a maintained catalog where every object, image, source, and database row can be traced back to evidence.

When the source material is multilingual, use `source-translation-workflow` first. That skill preserves raw text, working transcripts, manual translations, and excerpt statuses; this skill then links those translated excerpts into the catalog.

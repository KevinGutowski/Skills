...
**Sources/gaps:** [references/sources.md](references/sources.md) — Litt: Dive Club interview + nightmare-bicycle post + Ink & Switch essay; unresolved product/AI examples in [references/coverage-gaps.md](references/coverage-gaps.md). ⚠️ Dive Club quotes come from auto-transcripts with garbles corrected — the correction key lives in [references/sources.md](references/sources.md).
...
Field grounding for the Litt/Ink & Switch philosophy: by 2025–26 the malleable thesis stopped being speculative — working designers routinely make their own tools. Full quotes, names, and video IDs: [references/sources-and-concepts.md](references/sources-and-concepts.md).

- **Tools-of-one and tooling-gravity:** building for your exact needs delivers the per-person fit malleability promises — and because output gravitates toward what tooling makes easy, owning your tools shapes your work distinctively (a moat for your brand).
- **Build the tool, not the output:** AI's best role is helping you build or learn the tool, not jumping to the end result — Litt's AI-as-teacher-of-primitives stance in production use, and a 100×-iteration reframe for prompting. The coding bottleneck Litt says LLMs removed is, empirically, removed: first tools are deceptively simple to make.
- **Generative-design precedent** (Gross et al., p5.js): the stance predates LLMs — abstract into rules, repetition, controlled randomness, and parameters; the computer becomes a tireless variation engine while the designer evaluates and edits the abstraction. Guardrail: randomness works when "limited and applied in measured doses."
- **At scale:** a majority of surveyed designers have built a personal workflow tool; Ramp's ~1,500 vibe-coded internal tools are mostly throwaway, with keepers spreading organically — communal creation plus jig-grade disposability at company scale; platform vendors (Shopify) now design malleable, context-morphing interfaces for reshaping rather than against it; two-hour personal apps exist far below any commercial-viability line because the cost fell to an evening.

...
**Sources** — FastRuby.io/OmbuLabs posts + their open-sourced agent skills:
- *"Our Rails Upgrade Methodology as Claude Code Skills" (Tagwerker, 2026). https://www.fastruby.io/blog/open-source-claude-code-skill-for-rails-upgrades.html · repos: github.com/ombulabs/claude-code_rails-upgrade-skill, claude-code_dual-boot-skill*
- *"How to Dual Boot Rails" (Tagwerker, 2019, rev. 2025). https://www.fastruby.io/blog/upgrade-rails/dual-boot/dual-boot-with-rails-6-0-beta.html*
- *"How We Fix the Broken Build During a Rails Upgrade" (Lapham, 2026). https://www.fastruby.io/blog/fix-broken-build.html*
- *"How to Choose a Gem Wisely" (Torres & Tagwerker, 2026). https://www.fastruby.io/blog/how-to-choose-a-ruby-gem.html*
...
- **`rails-testing`** — a fast, reliable suite is this method's precondition (green before starting) and its instrument (the triage works through the suite).
- **`dhh-style`** / **`layered-rails`** — "keep that Gemfile as close to the original one as possible" (37signals) and the trusted gem map (Evil Martians) are the two schools' answers to the same risk this skill's vetting checklist measures.
- **`optimizing-rails`** — GC/JIT tunings can become counterproductive across Ruby versions — re-run performance experiments after upgrading.

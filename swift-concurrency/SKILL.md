...
- **`swiftui-identity`** — its dependency graph is the destination of the "coalesce before crossing isolation" rule; debug body-re-run storms there, throttle the firehose here.
- **`swiftui-lazy-stacks`** / **`swiftui-layout`** / **`swiftui-animation`** — UI-framework layers; this skill governs the async/data layer feeding them.
- **`agentic-coding`** — "only semantically meaningful conformances" and "no unstructured tasks in mainline flow" make good project-rules entries for AI-assisted Swift codebases.

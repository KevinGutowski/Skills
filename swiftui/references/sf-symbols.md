...
*Scope: Uses SF Symbols well — symbols as typographic objects (point sizes, weights, scales), rendering modes, Variable Color/Draw, symbol animations, custom-symbol annotation. Use when configuring symbols next to text, picking a rendering mode, animating a symbol, or building custom symbols. Based on WWDC sessions. Triggers: SF Symbols, systemName, symbolRenderingMode, variable color, symbolEffect, magic replace, custom symbol.*
...
See `sf-symbols/evolution-and-examples.md` for the version-by-version history, threshold math, and the worked custom-symbol examples (teacup, cupcake badge, kitchen timer).

...
- **Sheets & modal routing** (Thomas Ricouard — @dimillian, Ice Cubes; practitioner guidance, not Apple): prefer **`.sheet(item:)` over `.sheet(isPresented:)` whenever the sheet presents a selected model** — the closure receives the non-optional item, so there is never an `if let` inside the sheet body (that branch is a structural-identity trap and an empty-sheet-on-nil bug waiting to ship). **Sheets own their actions:** read `@Environment(\.dismiss)` and any stores *inside* the sheet and call `dismiss()` internally, rather than threading `onCancel`/`onConfirm` closures through layers. At app scale, centralize modal routing: one `Identifiable` `SheetDestination` enum describing every modal (share one `id` across mutually-exclusive editor-style flows so only one can be active), a `withSheetDestinations(sheet:)`-style modifier at the root switching enum → view, and a router object in the environment so any child can set `router.presentedSheet` — inject the router *after* the sheet modifier so modal content inherits it, and keep destinations lightweight (ids, not heavy state). Anti-patterns this replaces: **N boolean flags for mutually exclusive modals** (the enum makes impossible states unrepresentable; don't mix `isPresented:` and `item:` for one concern) and **giant views mixing routing, networking, and layout in one body** — routing is among the first responsibilities to extract. (His third anti-pattern, `AnyView` as a composition escape hatch, is mechanized in [swiftui-identity.md](swiftui-identity.md).) *API era: `sheet(item:)` is long-standing; the `@Observable` router shape assumes iOS 17+ — map to `ObservableObject` per [swiftui-identity.md](swiftui-identity.md)'s ownership table on iOS 16.*
...
- [ ] Modals item-driven — `sheet(item:)`, no `if let` in sheet bodies, no boolean-flag piles; sheets call `dismiss()` themselves; app-scale routing through one `SheetDestination` enum?

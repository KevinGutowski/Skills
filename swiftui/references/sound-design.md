...
**Implementation shape** (Thomas Ricouard — @dimillian, Ice Cubes; practitioner guidance, not Apple): centralize triggers in one `HapticManager`-style utility **gated on user preference and hardware support** — never assume a Taptic engine is present, and honor an off switch, the same law as sound's — with **distinct haptic types per UX moment** (selection change vs. success/error notification vs. intensity-scaled refresh) rather than one generic tap everywhere. Fire haptics right next to the user action, not deep in data layers, and never in rapid succession — which is Utility, above, enforced in code.
...
- **`ux-writing` (ui-voice-and-tone)** — sound is tone without words; a celebratory chime and a clinical alert follow the same dial logic as copy.

...
*Scope: Builds animations, transitions, and visual effects in SwiftUI — springs by duration+bounce, navigation and scroll-driven transitions, custom Transitions, TextRenderer and shader effects. Use when implementing SwiftUI motion, tuning springs, animating from gestures, or debugging transitions. Based on Apple WWDC sessions. Triggers: withAnimation, spring bounce, Animatable, navigationTransition, scrollTransition, visualEffect, keyframeAnimator.*
...
- **Which matched tool** (Thomas Ricouard — @dimillian, Ice Cubes; practitioner guidance, not Apple): `matchedTransitionSource` + `navigationTransition(.zoom)` for *cross-screen* continuity (thumbnail → viewer/detail/sheet); `matchedGeometryEffect` with a shared `Namespace` for *in-place* morphs within one hierarchy (a control changing shape under `withAnimation`). Either way the matched ids must be **stable — never freshly-generated UUIDs** — or the match silently breaks ([swiftui-identity.md](swiftui-identity.md)'s identifier law applied to transitions); and keep source/destination sizes and shapes roughly compatible or the zoom reads as jarring. *Staleness: his cookbook tags this pairing iOS 26-era; the zoom APIs shipped with iOS 18 per the WWDC source above — verify availability against current docs for your deployment target.*
...
- ⚠️ In **lazy stacks**, transforms that push a view out of its natural frame break visibility — see [swiftui-lazy-stacks.md](swiftui-lazy-stacks.md) Rule 3 before shipping a scroll effect.
...
See `swiftui-animation/code-patterns.md` for all verbatim samples (zoom, bridging, gestures, carousel/parallax, mesh gradient, Twirl, the full TextRenderer, and the Ripple shader + modifier).

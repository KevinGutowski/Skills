---
name: widget-design
description: "Design home-screen widgets that are personal, informational, and contextual — content strategy per size (small = one idea + single tap target; medium/large earn their space), configuration instead of cramming, Smart Stack relevance, layout rules (margins, concentric corners, placeholders, light/dark), and the don'ts (no logos, app names, instructions, or timestamps). Use when designing or reviewing a widget, deciding sizes or content strategy, configuring editing/intents, or auditing glanceability. Based on Apple WWDC 2020 session 10103 (iOS 14 foundation — principles hold; check current WidgetKit docs for lock-screen, rendering modes, and iOS 17+ interactive widgets). Triggers: widget design, WidgetKit, widget sizes, smart stack, widget configuration, placeholder, glanceable, widget margins."
---
...
- **`apple-typography`** / **`sf-symbols`** — the type and symbol mechanics inside the widget.

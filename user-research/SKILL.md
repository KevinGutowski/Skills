...
Three-book synthesis — Erika Hall, *Just Enough Research* (scoping, rigor, bias); Steve Portigal, *Interviewing Users* (interview craft); Tomer Sharon, *Validating Product Ideas* (lean method selection, behavioral validation) — plus supporting sources on validation/metrics, accessibility recruiting, content testing, dissemination, and interview field notes.
Full bibliography and section map → `references/sources.md`; unresolved method/evidence gaps → [references/coverage-gaps.md](references/coverage-gaps.md).
...
- `references/field-notes.md` — verbatim interview/podcast texture behind §5: empathy transfer (Goldin), stake-bearing dogfooding (D'Arcy), generative dogfooding and PMF-as-surprise (Soleio)
...
Sten's field-guide addition is a useful plain-language operator: **TEDW** — start follow-ups with **Tell me about**, **Explain**, **Describe**, or **Walk me through**. Use it especially with stakeholders who want to jump to solutions; it keeps them painting the situation instead of approving your framing.
...
**Engineer the empathy transfer** (Ron Goldin, Dive Club). Findings the org merely *reads* don't change behavior — design the delivery so people *feel* them; for the people who'll never go to the field, bring the field to them, uncompressed (Portigal's bring-colleagues doctrine, §2.12, extended).

**Dogfooding only counts with real stakes** (Polly D'Arcy, Dive Club). Staging walkthroughs are evaluative theater; stake-bearing use — your own money — is behavioral data on yourselves (§2.2 applied inward). Team rule: if the company won't use it with its own money, it's not good enough.

**Dogfooding is also generative — it can reveal the opportunity, not just the bugs** (Soleio, Design MBA). Treat your own daily friction as observational data with a generative payload (§1): the gap between tool assumptions and observed practice is a product thesis.

**PMF reads as surprise, not approval** (Soleio, Dive Club — Mike Maples's *Pattern Breakers* frame). The signal a want-test actually hit is magic-trick surprise; polite satisfaction is the anti-signal, and ambiguous validation results are negative results. Let responses redirect what the product *is*.

Full quotes and case texture for these four → [references/field-notes.md](references/field-notes.md).

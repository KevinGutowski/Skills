---
name: hig
description: Look up Apple's Human Interface Guidelines — the canonical, always-current reference for platform conventions, component specs, and design patterns on iOS/iPadOS/macOS/watchOS/tvOS. This is a *lookup* skill — it holds the HIG's topic map and a reliable fetch method (the JSON endpoint; the HTML pages are JS-rendered and fetch empty), not copied content. Use when a question needs Apple's authoritative guidance on a specific component (buttons, sheets, sliders, tab bars…), pattern (onboarding, undo, settings, modality…), input (gestures, keyboards, Apple Pencil…), or technology (CarPlay, Apple Pay, generative AI…) — especially anything the talk-derived design skills don't cover, or when exact current specs matter. Triggers: HIG, Human Interface Guidelines, Apple guidelines, what does Apple say about, platform conventions, component spec, alerts, action sheets, popovers, pickers, toggles, gauges, complications, CarPlay, watchOS design, tvOS design.
---
...
1. **A talk skill owns the topic** → use it first; pull the HIG page only for exact specs or platform-considerations detail (e.g. `apple-navigation-design` for tab-vs-sidebar reasoning, then `tab-bars` for current specs).
...
HIG-grounded siblings (deep dives; prefer them when they match): `design-principles`, `apple-navigation-design`, `apple-search-design` (search-fields, searching), `apple-typography` (typography), `sf-symbols` (sf-symbols), `apple-visual-accessibility` (accessibility, inclusion), `liquid-glass-design-system` (materials), `app-icon-design` (app-icons), `ios-brand-identity` (branding), `widget-design` (widgets, complications), `notification-design` (managing-notifications), `chart-experience-design` (charts, charting-data), `app-intents-design` (app-shortcuts, siri, snippets), `sound-design` (playing-audio, playing-haptics), `feature-discoverability` (onboarding, offering-help), `ai-experience-design` (generative-ai, machine-learning), `ui-voice-and-tone` (writing), `touch-interaction-design` (gestures, feedback).

...
HIG-grounded siblings (deep dives; prefer them when they match): `design-principles`, `apple-navigation-design`, `apple-search-design` (search-fields, searching), `apple-typography` (typography), `swiftui` (sf-symbols) (sf-symbols), `apple-visual-accessibility` (accessibility, inclusion), `liquid-glass-design-system` (materials), `app-icon-design` (app-icons), `ios-brand-identity` (branding), `swiftui` (widget-design) (widgets, complications), `notification-design` (managing-notifications), `chart-experience-design` (charts, charting-data), `app-intents-design` (app-shortcuts, siri, snippets), `swiftui` (sound-design) (playing-audio, playing-haptics), `feature-discoverability` (onboarding, offering-help), `ai-experience-design` (generative-ai, machine-learning), `ui-voice-and-tone` (writing), `swiftui` (touch-interaction-design) (gestures, feedback).

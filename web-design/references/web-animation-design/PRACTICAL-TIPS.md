...
- **Keys are an animation tool**: "when a component's key changes, React will treat that component as a different component, unmounting the existing component" — so changing a key *replays the mount animation* and, inside `AnimatePresence`, triggers the old child's exit + the new child's entrance. Stable keys = persistent state; changed keys = a deliberate remount transition. (SwiftUI's `.id()` analog lives in `swiftui-identity`.)

...
- Map to **Core Web Vitals**: LCP (loading), **INP** (responsiveness — superseded FID in 2024; the field version of *Instant*), CLS (layout stability — doctrine in `design-craft` (polish-principles) §17: preload over skeletons, content lands exactly where its placeholder was). Google judges CWV at the **75th percentile of field data** (CrUX) — "75% of page visits must meet 'Good' thresholds" (Osmani, web-quality-skills); a clean lab run on your machine proves nothing about the field number that ranks you.
...
- **Anticipate the next page, not just the next click** (Osmani, web-quality-skills): "the LCP a user actually experiences is dominated by *the next page they navigate to*, not the one they landed on" — Speculation Rules `prerender` on likely-next links collapses that LCP to ~0ms. The `eagerness` ladder, cheapest → most aggressive: `conservative` (pointerdown) → `moderate` (~200ms hover) → `eager` (link in viewport) → `immediate` (page load); start at `moderate`. Each prerender is roughly a full page load, so scope `where` patterns (exclude logout/checkout), and gate analytics/ads/side effects on `document.prerendering` / the `prerenderingchange` event — load-time code fires at prerender start, not at navigation. Chromium-only: a progressive enhancement, never a dependency.
- **Perceived performance is real performance**: a faster spinner makes identical load times feel faster; optimistic states, specific progress copy, and choreographed latency (confirm steps that absorb API time) all buy real felt speed. (Animation-side specifics live in [`web-animation-design`](web-animation-design.md).)
...
- **[`web-animation-design`](web-animation-design.md)** — perceived-speed and animation-performance rules (transform/opacity only, blur budgets, read/write batching + FLIP, scroll/view timelines, layer promotion, View Transitions) live there; this skill supplies the frame budgets and process that enforce them.
...
> **Staleness note (Kevin's rule):** framework specifics (Next.js app router, React `useTransition`/`useOptimistic`/compiler, router-cache behavior) are 2024–25-era — verify against current docs. INP replaced FID as a Core Web Vital in March 2024. Speculation Rules API support and eagerness semantics are Chromium-era specifics — re-check browser coverage before relying on them. The frame budgets, fluid-UI principles, triage model, and process rules are the durable layer.

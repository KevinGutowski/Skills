...
description: Design notifications worth receiving — when a notification earns the interruption (value to the user, not the app), respecting attention (timing, frequency choices, granular settings, quiet/provisional delivery), meaningful grouping, and the self-contained package (Short Look answers what; Long Look adds rich context; quick actions complete the task without launching the app), including Apple Watch. Use when designing or reviewing notification strategy, deciding what to send and when, writing permission flows, threading, designing rich/actionable notifications, or auditing notification fatigue. Based on Apple WWDC sessions 817 (2017) and 806 (2018). Triggers: notification design, push strategy, notification fatigue, deliver quietly, provisional authorization, grouping, threadIdentifier, rich notification, long look, when to notify.
---

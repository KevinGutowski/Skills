...
- **Stack choices carry measurable agent-cost consequences** (Briggs, Jun 2026 experiment): colocation beats indirection for agent-editable styling — Tailwind-style colocated classes cost roughly half the agent tokens of separate stylesheet files, because the agent edits in place instead of reading and modifying across files. Audit where your stack forces agents to read-then-edit across files, and budget stack decisions with agent token spend in view. (Experiment design and full numbers: [ai-enablement/sources.md](ai-enablement/sources.md).)

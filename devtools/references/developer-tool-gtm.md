...
- **[devtool-interface-design.md](devtool-interface-design.md)** — makes the tool worth adopting (including the a-ha onboarding and AX discovery rules that feed this reference's organic-signups engine); this reference gets it adopted.
- **`write-clear-prose`** — sentence-level mechanics; the devtool copy layer (clichés, numbers, scannability) lives here.
- **`ux-writing` (naming-features-and-labels)** — names features; CTA wording (conversion-tested) lives here.
- **[oss-strategy.md](oss-strategy.md)** — READMEs and OSS popularity; "open source users are users, too" is the bridge.

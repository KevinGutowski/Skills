...
*Scope: Writes and reviews error messages and failure-state copy — any in-app text shown when something goes wrong. Use for error toasts, dialogs, form validation, 404/500 pages, retry prompts, or API/payment/auth failure copy, or reviewing error copy for tone, clarity, or blame. Triggers: error message, error copy, validation message, something went wrong, failure state, retry prompt.*
...
- **[`ui-voice-and-tone`](ui-voice-and-tone.md)** sets the product's overall voice and where the serious/clinical tone dial sits; this skill owns the failure-state specifics (the four anti-patterns, the five essentials). Inherit the voice from there; apply the failure rules here.

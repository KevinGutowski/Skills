...
- **[`linear-settings-copy`](linear-settings-copy.md)** decides *where a setting lives* in the IA and writes the full row (title + helper text + control/option labels). **This skill** decides *what to call* the feature, value, or option in the first place, and how to defend that choice. They compose: use this skill to pick the word, then [`linear-settings-copy`](linear-settings-copy.md) to place it and write the surrounding copy. When a task is purely "name this setting/option label," reach for this skill's criteria; when it's "lay out this settings page," reach for [`linear-settings-copy`](linear-settings-copy.md).
- **[`error-messages`](error-messages.md)** handles failure-state copy (what to say when something breaks). No overlap, but a feature's *name* should still appear correctly inside an error message.
...
- **[`ui-voice-and-tone`](ui-voice-and-tone.md)** defines the app's overall writing voice and the microcopy around a name; this skill picks the name itself. Set the voice there, choose and defend the word here. (The exercises rhyme — adjective-clustering vs. Think/Feel/Do — but produce different outputs.)
...
See [naming-features-and-labels/worked-examples.md](naming-features-and-labels/worked-examples.md) for the full case studies (Apple Cash, gym plans, Visited Places, Memories, Enhance Dialogue, AutoMix) with the reasoning spelled out.

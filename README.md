...
| `rails` | ALL Ruby/Rails work — one consolidated skill; body routes to reference files: official Rails Guides lookup, official Rails reference apps (Campfire, Writebook, Fizzy), four schools (dhh-style vanilla default, layered-rails, packwerk, rails-event-sourcing), Active Record associations, webhooks, migrations, security/multitenancy, jobs, both testing schools, Hotwire/realtime, websocket scale, Docker dev, performance, upgrades, Inertia, ruby-refactoring |

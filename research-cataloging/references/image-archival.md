...
**Don't use for:** routine web-export sizing (a normal export pipeline handles that), images already small (<~2 MB — not worth it), or hard-archival/legal masters that must be bit-exact forever (keep those untouched). For research-bundle/provenance archiving of *web* sources, use [archival-research.md](archival-research.md) instead.

...
## Relationship to other references and skills

- **[archival-research.md](archival-research.md)** — sibling discipline: that reference governs *provenance and evidence-grade capture* for dossiers/archives; this one governs *coverage* when the goal is distillation. A big mine often uses both: sweep for coverage, archive what's load-bearing.
- **`deep-research`** (skill) — for open-web questions (unbounded space, search-driven); this discipline is for *bounded* sources where "all of it" is a meaningful target.
- **[catalog-reconciliation-research.md](catalog-reconciliation-research.md)** — downstream consumer: its evidence tiers assume the sweep actually happened.

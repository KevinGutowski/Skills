...
- **Color math** (Evil Martians): OKLCH makes color *programmable* — rotate hue at fixed L/C for cohesive accessible families; hover/pressed = arithmetic on L; dark theme = mirror lightness, keep chroma/hue. Change the brand color, everything recomputes (→ `oklch-skill`). Baldwin's **Leonardo** (leonardocolor.io) generates palettes *from target contrast ratios* — contrast-first beats hand-checking pairs.

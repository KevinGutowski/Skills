...
This skill represents a **bold/distinctive aesthetic theme** — deliberately different from the restrained "invisible polish" execution themes inside `design-craft` (especially its emil-kowalski reference) and `web-design` (web-animation-design). These themes shouldn't be mixed: a single project should pick one direction and commit.
...
- **The Design Read** (Leon, taste-skill v2): declare one line before generating — "Reading this as: \<page kind> for \<audience>, \<vibe>, leaning \<system/aesthetic>." Ask at most ONE clarifying question, only when plausible reads genuinely diverge; otherwise state the read and proceed.
- **Physical-scene test** (Bakaus, Impeccable): dark vs light "is never a default" — write one sentence of who uses this, where, under what ambient light, in what mood. "If the sentence doesn't force the answer, it's not concrete enough. Add detail until it does."
- **Category-reflex check, two altitudes** (Bakaus, Impeccable): theme+palette guessable from the category alone = the first training-data reflex; aesthetic family guessable from category-plus-anti-references ("fintech that's not navy-and-gold → terminal-native dark mode") = "the trap one tier deeper." Rework until neither answer is obvious.
...
- **Color & Theme**: Commit to a cohesive aesthetic. Use CSS variables for consistency. Dominant colors with sharp accents outperform timid, evenly-distributed palettes. Choose a color *strategy* before choosing colors (Bakaus's commitment axis): Restrained (tinted neutrals + one accent ≤10%), Committed ("one saturated color carries 30–60% of the surface"), Full palette (3–4 named roles used deliberately), or Drenched ("the surface IS the color").
- **Motion**: Use animations for effects and micro-interactions. Prioritize CSS-only solutions for HTML. Use Motion library for React when available. Focus on high-impact moments: one well-orchestrated page load with staggered reveals (animation-delay) creates more delight than scattered micro-interactions. Use scroll-triggering and hover states that surprise. Reveal animations must enhance an already-visible default — class-gated visibility never fires in hidden tabs or headless renderers, "so the reveal never fires and the section ships blank" (Bakaus, Impeccable).
...
Content honesty has a name — the "Jane Doe effect" (Leon, taste-skill v2): generic names/avatars, fake-perfect numbers (`99.99%`, `1234567`), and startup-slop brand names (Acme, Nexus, Cloudly) are tells. Default: every number is real, explicitly labeled mock, or removed — "don't fake engineering precision the brand doesn't claim." Before shipping, self-audit every visible string for LLM tells (broken referents, mock-poetic section labels, forced wordplay): "AI-generated cute copy is worse than boring copy."
...
**Image sourcing** rides the same descending ladder (Leon, taste-skill v2): (1) an image-gen tool when the environment has one; (2) real placeholder photography with descriptive seeds (picsum `/seed/{descriptive-seed}/`); (3) clearly-labeled TODO slots plus a closing list of needed shots — never div-built fake screenshots ("the #1 LLM-design Tell") or sketchy hand-rolled SVG scenes as a fallback. "A pure-text page is not minimalism. It is incomplete work." For a strictly single-file, zero-network artifact, tier 3 plus code-native visuals is the floor — as with fonts, the ladder sets how low you may go, not where to start.
...
- **Official-system briefs install the official package** (Leon, taste-skill v2): a brief that reads Microsoft/Google/IBM/Shopify/Atlassian/GitHub/gov gets Fluent, Material 3, Carbon, Polaris, Atlaskit, Primer, or govuk-frontend/USWDS — "do not recreate its CSS by hand"; one system per tree. Aesthetics with no official package (bento, brutalism, Apple Liquid Glass) are hand-built approximations — label them as such.
- **Token-name test + domain protocol** (Damilola Jay, interface-design): read your CSS variables aloud — "`--ink` and `--parchment` evoke a world; `--gray-700` and `--surface-2` evoke a template." Reconcile with shadcn semantic conventions and the warm-paper tells below: semantic names (`bg-card`) at the system layer, world-evoking names at the brand layer; `--paper` on the default cream palette is the tell, not evocative naming itself. Before visuals, run the domain protocol: 5+ concepts from the product's world, 5+ colors from its physical world, one signature element "that could only exist for THIS product," 3 named clichés to reject — then strip the product name and check the proposal still identifies it.
...
- **Polish prompts need numbers** (Kevin Kold / @kvnkld, X Article 2064972712729292800, 2026): "smooth" is not buildable. Give the exact curve, duration, rise distance, blur amount, press scale, shadow layers, radius token, and state list. Lead with the design-token block and forbid one-off values. Iterate one concern at a time ("only the entrance," "only the shadow stack") so the agent does not compensate with unrelated changes.
...
## Checkable slop tells (as of 2026)

Dated deliberately — tells drift with training data; re-verify before enforcing. Numeric bands from Bakaus's Impeccable detector (measure, don't vibe):

- **Warm-neutral body bg**: OKLCH L 0.84–0.97, C < 0.06, hue 40–100 "reads as cream/sand/paper/parchment regardless of what you call it" — the saturated AI default. Warmth "is carried by accent + typography + imagery, not by body bg." Token names `--paper`, `--linen`, `--parchment` are tells in themselves.
- **Display type**: clamp() max ≤ 6rem ("above that the page is shouting, not designing"); letter-spacing floor ≥ −0.04em ("anything tighter and letters touch").
- **Cards**: radius caps at 12–16px (full-pill for tags/buttons only); ghost-card tell = 1px border + ≥16px-blur shadow on one element — pick border or shadow, never both as decoration.

Mechanical pre-flight counts (Leon, taste-skill v2) — countable before shipping:

- Eyebrows/kickers ≤ ceil(sections/3); count `uppercase tracking`-style labels mechanically.
- A third consecutive image/text zigzag section fails; a page of 8 sections needs ≥4 distinct layout families.
- Hero text stack ≤4 elements; trust micro-strips, pricing teasers, and avatar rows are banned from heroes — they move below.
- Bento cell count = real content count: an empty cell means "you planned wrong" — reshape the grid, don't pad it.

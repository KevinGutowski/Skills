...
8. **Leverage what users already carry; reward curiosity instead of instructing.** Build on knowledge users bring, curate the invisible layer that supports it, and let explorable details replace instruction (Wardle, Config 2024; Andy Allen).
9. **Design the share artifact.** Spoiler-free, narrates the user's journey, no self-promoting link-back — artifacts that don't self-promote get shared more (Wardle).
10. **Sequence trust from low stakes to high.** Never ask for the high-stakes commitment first; wrap novel tech in a familiar analog; diagnose *whose* trust is missing — sometimes users distrust themselves, not you (Jude, Config 2025).
11. **You have ~30 seconds — and name the private shame.** Deliver a first win before users retreat to the tool they know; naming a self-blamed pain creates instant resonance. Caveat: a simpler learning experience is not always the better UX (Feener, Browser Company).
12. **Invert the funnel — ask after value.** Radically streamline or fully invert signup; build canvases (multiple paths to the goal), not corridors (Zhang, Config 2024).
13. **Free play before signup.** Let people try before authenticating, and constrain that first-use surface so the free taste is a good one (Ryo Lu, Cursor, 2025). AI-native version: prove value at step one instead of asking questions (Emily Campbell → `ai-experience-design`, references/patterns.md).
14. **Delight has a measured conversion cost — spend it deliberately.** Emotional framing before signup can cost real sign-ups; pay only when the emotional frame is the product's edge, and design targeted, not generalized (Conde, Dive Club).
15. **Pattern fidelity for utilitarian flows.** Signup and setup are the follow-patterns zone; save invention for the product's signature moments (Jacoby, Dive Club).

Quote texture and case detail for rules 8–15: [field-notes.md](references/field-notes.md).
...
> **Staleness note:** the Hulick journey framework and copy rules are durable. Universal Rules 8–15 come from 2024–25 Config talks and Dive Club interviews; the product examples (Wordle, Waymo, Arc, TikTok, Cursor) and any numbers in them describe those products at that moment — verify before citing them as current behavior.
